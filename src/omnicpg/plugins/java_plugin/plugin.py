"""JavaPlugin — language plugin for analysing Java source files.

Supports Java source (``.java``), JSP pages (``.jsp``), and framework
configuration files (``.xml``).  Understands annotations and class
hierarchies from:

* **Spring 3.x** — ``@Controller``, ``@Service``, ``@Repository``,
  ``@RequestMapping``, ``@Autowired``, ``@Transactional``, etc.
* **Struts 1.x** — ``Action`` / ``ActionForm`` base-class detection and
  ``struts-config.xml`` parsing.
* **Hibernate** — ``@Entity``, ``@Table``, ``@Column``, and
  ``hibernate.cfg.xml`` / ``*.hbm.xml`` parsing.
"""
...
from omnicpg.plugins.java_plugin.interprocedural_dfg_builder import InterProceduralDFGBuilder
...
class JavaPlugin(LanguagePlugin):
    """Concrete :class:`LanguagePlugin` for Java projects.
    ...
    * :class:`CallGraphBuilder` — cross-file ``CALLS`` edges.
    ...
    def __init__(self) -> None:
        """Initialise the Java plugin with its builder components."""
        ...
        self._interprocedural_dfg_builder = InterProceduralDFGBuilder()

    # ── LanguagePlugin interface ──────────────────────────────────────────

    @property
    def supported_extensions(self) -> list[str]:
        """Return Java-family source and configuration extensions."""
        return [".java", ".jsp", ".xml", ".properties"]
    ...
    def parse_to_ast(
        ...
        """Parse Java *source_code* into AST nodes and edges."""
        from omnicpg.models.analysis_level import AnalysisLevel
    ...
    def build_call_graph(
        ...
        """Derive ``CALLS`` edges across all analysed Java files."""
    ...
    def build_interprocedural_dfg(
        ...
        """Derive inter-procedural ``REACHES`` edges for Java."""
        if all_edges is None:
            return []
        return self._interprocedural_dfg_builder.build(all_nodes, all_edges)
